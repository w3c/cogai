...
A starting point would be to model forgetting with a leaky capacitor, and learning as the injection of charge, where the amount of charge is smaller for short time intervals, but levels out as the interval increases. This can be modelled with the logistic function. For recall, we return the strongest matching chunk after multiplying the chunk strength by a random factor that is in the range 0 to plus infinity, and averages around 1. The factor is computed as the Math.exp(x) where x is from a gaussian distribution centred on zero with a standard deviation of 0.5. Other ideas are welcomed!
