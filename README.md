...
Of course there are many challenges to address before this technology can become widespread. This includes surface passivation and physical encapsulation within a protective matrix, as well as connectivity to photonic chips compared to the ease of bonding wired connections.  We are also likely to need new architectures for neural networks that feature continual learning through continual prediction, and locally generated weight adjustments at a layer by layer basis, replacing today’s gradient-based back propagation. Another motivation is that adversarial attacks suggest that today’s AI is different from human cognition, e.g. adding some carefully chosen noise to an image of a panda causes the model to misclassify it as a gibbon with a high degree of confidence, despite the noise being imperceptible to the human eye. Similar considerations apply to prompt injection jail breaks that evade the alignment training for LLMs.
