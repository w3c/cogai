...
WebNNM uses a ping/pong approach to parameters and their momentum to keep the tensors in the NPU or GPU during training, only reading the updated parameters after the final epoch. The implementation uses the [Refined Lyon optimiser](https://www.nature.com/articles/s41598-025-07112-4) which offers faster convergence using less memory that AdamW.  For inference, the parameters are baked into the graph as constants, reducing the time to prepare the next batch.
