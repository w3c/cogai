...
Developers will often want to apply transfer learning to fine tune a pretrained model or to adapt it to a new purpose. WebNNM seeks to make this easy for newcomers whilst giving experts all the control they need. The library computes a depth metric based upon the path length from any DAG node to any input node, and likewise to any output node. Less experienced developers choose what fraction of the model to freeze. The learning rate for the rest ramps up in discrete steps to the baseline. Experts can override that by setting learning rate multipliers as layer options. For multimodal models you might have a much longer pipeline for video than audio. The depth metric therefore avoids cross attention to remain within a given modality.

|User Level|Interaction|Underlying Metric|
|----------|-----------|-----------------|
|**Novice**|_Freeze the first 50% of the model_|**Plasticity Slider** (Using Path Ratio)|
