...
Developers will often want to apply transfer learning to fine tune a pretrained model or to adapt it to a new purpose. WebNNM seeks to make this easy for newcomers whilst giving experts all the control they need. The library computes a depth metric based upon the maximum path length from each DAG node to any DAG input node, and likewise to any output node. Developers choose what proportion of the layers to lock down. The learning rate for the rest ramps up to the baseline following a sinusoidal profile with a limited number of discrete steps. Experts can override that by setting explicit learning rate multipliers as layer options. For multimodal models you might have a much longer pipeline for video than audio. The depth metric therefore avoids cross attention to remain within a given modality.
