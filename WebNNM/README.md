...
The starting point for training models is to pick the initial values for the model's trainable parameters. This is done using stochastic algorithms selected according to the local activation function. Unfortunately, this may still result in a high initial loss that stubbornly refuses to train nicely. Work is underway to generate multiple sets of initial parameter values using different random number seeds, and to apply metrics to select the most promising set to pass to the training loop. The computation of the metrics involves a dedicated scouting graph together with running the training graph exactly once. The metrics include: *Loss*, *Gradient Ratio*, *Dead Ratio*, and *Delta Loss*. These check for vanishing gradients, activation saturation (dead neurons) and exploding gradients.
